...
export function ReceiptScreen({
	...
	const totalInput = formatUsd(
		formatBaseUnits(record.plan.totalInputBaseUnits, 6),
	);
		...
		<main className="receipt-page">
			...
			<section className="receipt-ledger">
				...
				{selected.map((candidate) => {
					...
					const fullOutput =
						isSuccess && output
							? formatBaseUnits(output.amountOutBaseUnits, candidate.decimals)
							: undefined;
					return (
						<div className="receipt-row" key={candidate.assetId}>
							...
							<div className="receipt-asset">
								...
								<small>
									{quote
										? `${formatUsd(formatBaseUnits(quote.amountInBaseUnits, 6))} allocation`
										: "Allocation unavailable · USDC"}
								</small>
							</div>
							<div
								className={
						...
						<div className={demoMode ? "demo-disclosure" : "live-disclosure"}>
							{demoMode
								...
								: record.submissionMode === "BATCH"
									? `Live settlement is verified from the atomic Solana operation and output-token transfers to your wallet.`
									: `Live settlement is verified per Solana transaction and output-token transfer to your wallet.`}
						</div>
					</div>
				</details>
...
function executionProviderLabel(provider: ExecutionRecord["plan"]["provider"]) {
	return provider === "ZERO_EX" ? "0x" : "Jupiter";
}
		...
		return {
			...
			description:
				submissionMode === "BATCH"
					? "Your wallet broadcast one atomic operation. Waiting for Solana settlement."
					: "Your wallet broadcast independent swaps. Waiting for Solana settlement.",
		};
	}
	if (status === "SETTLED") {
		return demoMode
